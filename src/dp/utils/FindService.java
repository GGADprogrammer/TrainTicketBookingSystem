...
public class FindService {
	...
	//用户使用
	public static List<Piao> getPiao(ResultSet rs,int vipLevel){
		List<Piao> alist=new ArrayList<>();
		if(rs!=null){
			try {
				rs.beforeFirst();
				while(rs.next()){
					...
					a.setSzdname(rs.getString("saddr"));
					a.setPtype(rs.getString("ptype"));
					//String转double,进行运算再转回string
					double price = Double.valueOf(rs.getString("pval"));
					...
					a.setPval(Double.toString(price));
	...
	//获取会员结果集
	public static List<Vip> getVip(ResultSet rs){
		List<Vip> alist=new ArrayList();
		if(rs!=null){
			try {
				rs.beforeFirst();
				while(rs.next()){
					Vip a=new Vip();
					a.setId(rs.getInt("id"));
					a.setVipLevel(rs.getInt("VipLevel"));
					alist.add(a);
				}
			} catch (SQLException e) {
				// TODO Auto-generated catch block
