...
public class DbConn {
	...
	public DbConn(){
		//数据库连接字符串初始化
		url="jdbc:mysql://localhost:3306/hcdpsys?workuseUnicode=true&characterEncoding=utf8&serverTimezone=GMT";
		//账号密码初始化
		username = "root";
		passawd = "384145967";
		//数据库连接初始化
	...
	//判断Vip
	public int  getLevel(Object userid) {
		int level=0;
		String sql="select * from vip where id="+userid;
		ResultSet rs=query(sql);
		try {
			if(rs.next()){
