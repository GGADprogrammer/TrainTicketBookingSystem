...
public class Init extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if("piao".equals(tname)){
			String ids=request.getParameter("tid");
			if("1".equals(flgs)){
				sql="select t.*,traininfo.tname from(select tmp.*,zdname as eaddr,tname,traintype from (select piao.*,zdname as saddr from piao,zhandian where piao.szdid=zhandian.id) tmp,zhandian,traininfo where tmp.tid=traininfo.id and tmp.ezdid=zhandian.id)t join traininfo  on t.tid=traininfo.id";
				hp.setValue("sql", sql);
			}else{
				sql=hp.readValue("sql");
			}
